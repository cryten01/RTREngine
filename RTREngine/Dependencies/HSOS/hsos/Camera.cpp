...
Camera::Camera() : m_Position(0.0f, 15.0f, -20.0f), m_Target(0.0f, 0.0f, 0.0f), m_Up(0.0f, 1.0f, 0.0f), 
m_LastMouseX(-1), m_LastMouseY(-1), m_Panning(0, 0, 0), m_Zoom(0, 0, 0), m_Rotation(0, 0, 0), 
...
void Camera::resize(unsigned width, unsigned height)
{
	...
	m_ProjMatrix.perspective(static_cast<float>(M_PI)*65.0f / 180.0f, static_cast<float>(width) / static_cast<float>(height), 0.045f, 1000.0f);
	windowHeight = height;
	windowWidth = width;
}
